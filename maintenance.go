// Package maintenance_warden provides a Traefik plugin to redirect traffic to a maintenance page
// while allowing requests with a specific header to bypass the redirection
package maintenance_warden
...
func CreateConfig() *Config {
	return &Config{
		MaintenanceService: "http://maintenance-page-service",
		MaintenanceFilePath: "",
...
func New(ctx context.Context, next http.Handler, config *Config, name string) (http.Handler, error) {
	...
	if config.MaintenanceFilePath != "" {
		...
		return nil, fmt.Errorf("either maintenanceService or maintenanceFilePath must be specified")
	}
